...
export const Onboarding: React.FC<OnboardingProps> = ({ onComplete, onAdminLogin, onExit }) => {
  const [formData, setFormData] = useState<Partial<UserProfile>>({ ...INITIAL_USER_STATE });
  const [step, setStep] = useState(1);
  const totalSteps = 8; 

  // Image State
  const [headerImage, setHeaderImage] = useState("https://res.cloudinary.com/dv1z6p82q/image/upload/v1740924976/sheep-logo_copy_rmq5l1.png");
             ...
             <div className="grid grid-cols-1 gap-4">
               {[
                 ...
                 <button
                   ...
                   className={`p-6 rounded-2xl border-2 transition-all flex flex-row items-center justify-center gap-4 hover:scale-[1.02]
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
               {INDUSTRY_OPTIONS.map((opt) => (
                 <button
                   ...
                   className={`p-4 rounded-2xl border-2 transition-all flex flex-col items-center justify-center gap-2 hover:scale-105
               ...
               <button
                   ...
                   className={`p-4 rounded-2xl border-2 border-dashed border-slate-300 transition-all flex flex-col items-center justify-center gap-2 hover:bg-slate-50 hover:border-indigo-300
  ...
  return (
    <div className="h-screen w-full flex flex-col bg-white overflow-hidden">
      ...
      {/* HEADER: Close Button & Admin & Progress Bar */}
      <div className="relative z-20 bg-white">
          <div className="flex justify-between items-center p-4">
            <button 
                onClick={onExit}
                className="p-2 rounded-full bg-slate-100 hover:bg-slate-200 text-slate-600 transition-colors"
                title="סגירה וחזרה למסך הבית"
            >
            ...
            <div className="text-xs font-bold text-indigo-900 bg-indigo-50 px-3 py-1 rounded-full border border-indigo-100">
          ...
          {/* Progress Line */}
          <div className="w-full h-1 bg-slate-100" dir="ltr">
            <div 
      ...
      {/* RECTANGULAR EDITABLE IMAGE BANNER */}
      <div 
        className="w-full h-40 sm:h-56 bg-slate-100 relative group overflow-hidden flex-shrink-0 cursor-pointer border-b border-slate-200 flex items-center justify-center"
      ...
      {/* FOOTER: BUTTONS STACKED 100% WIDTH */}
      <div className="p-6 bg-white border-t border-slate-100 shadow-[0_-5px_20px_rgba(0,0,0,0.05)] z-20">
        <div className="max-w-xl mx-auto flex flex-col gap-3 w-full">
            <button
                onClick={handleNext}
                disabled={!isStepValid()}
                className={`w-full py-4 rounded-2xl font-black text-lg shadow-lg transition-all duration-300 transform active:scale-95
                ${isStepValid() 
                    ? 'bg-indigo-600 text-white hover:bg-indigo-700 hover:shadow-indigo-500/30' 
                    : 'bg-slate-200 text-slate-400 cursor-not-allowed shadow-none'}`}
            >
                {step === totalSteps ? 'סיימתי, בואו נתחיל! 🚀' : 'המשך לשלב הבא'}
            </button>

            {step > 1 && (
                <button
                onClick={handleBack}
                className="w-full py-3 rounded-2xl text-slate-500 font-bold hover:bg-slate-50 transition-colors border border-transparent hover:border-slate-200"
                >
                חזרה לשלב הקודם
                </button>
            )}
